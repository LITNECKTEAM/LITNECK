...
local getversion = https.request('https://api.th3LITNECK.com/version/')
...
#!/usr/bin/env bash
cd $(cd $(dirname $0); pwd)
if ! screen -list | grep -q "LITNECK"; then
screen -S LITNECK lua ./inc/BOT.lua
fi
screen -X -S LITNECK kill
screen -S LITNECK lua ./inc/BOT.lua
]])
file:close()
...
_info = load_config()
LITNECK = _info.TOKEN:match("(%d+)")
our_id = tonumber(LITNECK)
bot_username = redis:get(LITNECK..":username")
...
print('\n\27[0;32mPlugins is : '..#list_plug..' Are Active.\n\n\27[0;31mThe SOURCE RUN IS TH3LITNECK v'..redis:get(LITNECK..':VERSION')..'...\n\27[m')
end
load_plugins()
...
return false end 
if not (tostring(msg.chat_id_):match('^-100') or tostring(msg.chat_id_):match('-')) and redis:get(LITNECK..':mute_pv:'..msg.sender_user_id_) then
...
if redis:get(LITNECK..'mute_text'..msg.chat_id_) then --قفل الدردشه
