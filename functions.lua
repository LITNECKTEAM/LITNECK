...
tdcli_function({ID ="GetChat",chat_id_=GroupID},function(arg,data) redis:set(LITNECK..'group:name'..GroupID,data.title_) end,nil)
return redis:get(LITNECK..'group:name'..GroupID)
end
function rem_data_group(id_group)
redis:del(LITNECK..'group:add'..id_group) redis:srem(LITNECK..'group:ids',id_group) redis:del(LITNECK..'group:name'..id_group) redis:del(LITNECK..'lock_link'..id_group) redis:del(LITNECK..'lock_id'..id_group) redis:del(LITNECK..'lock_spam'..id_group) redis:del(LITNECK..'lock_webpage'..id_group) redis:del(LITNECK..'lock_markdown'..id_group) redis:del(LITNECK..'lock_flood'..id_group) redis:del(LITNECK..'lock_bots'..id_group) redis:del(LITNECK..'mute_forward'..id_group) redis:del(LITNECK..'mute_contact'..id_group) redis:del(LITNECK..'mute_location'..id_group) redis:del(LITNECK..'mute_document'..id_group) redis:del(LITNECK..'mute_keyboard'..id_group) redis:del(LITNECK..'mute_game'..id_group) redis:del(LITNECK..'mute_inline'..id_group) redis:del(LITNECK..'num_msg_max'..id_group) redis:del(LITNECK..'extimeadd'..id_group) redis:del(LITNECK..'CheckExpire::'..id_group)redis:del(LITNECK..'admins:'..id_group)redis:del(LITNECK..'whitelist:'..id_group)redis:del(LITNECK..'owners:'..id_group)redis:del(LITNECK..'klmamn3'..id_group)redis:del(LITNECK..'lock_edit'..id_group) redis:del(LITNECK..'lock_link'..id_group)redis:del(LITNECK..'lock_tag'..id_group)redis:del(LITNECK..'lock_username'..id_group) redis:del(LITNECK..'lock_pin'..id_group) redis:del(LITNECK..'lock_bots_by_kick'..id_group) redis:del(LITNECK..'mute_gif'..id_group) redis:del(LITNECK..'mute_text'..id_group) redis:del(LITNECK..'mute_photo'..id_group) redis:del(LITNECK..'mute_video'..id_group) redis:del(LITNECK..'mute_audio'..id_group) redis:del(LITNECK..'mute_voice'..id_group) redis:del(LITNECK..'mute_sticker'..id_group) redis:del(LITNECK..'mute_tgservice'..id_group)redis:del(LITNECK..'welcome'..id_group)redis:del(LITNECK..'replay'..id_group) redis:del(LITNECK..'lock_woring'..id_group)
local names = redis:hkeys(LITNECK..'replay:'..id_group)
for i=1, #names do redis:hdel(LITNECK..'replay:'..id_group,names[i]) end
end
...
redis:set(LITNECK..':WITTING:ON',msg.to.id)
redis:setex(LITNECK..":RUN_BOT",5,true)
os.execute('rm -fr ../.telegram-cli') sleep(0.2) os.exit()
end
local website = 'https://th3LITNECK.com/join/?id='
...
redis:set(LITNECK..'linkgp'..GroupID,linkx)
return linkx
end
...
return req.Ch_Member.ch_TH3LITNECK
end
...
if redis:sismember(LITNECK..':SUDO_BOT:',msg.from.id) then var = true end return var
end
function is_sudo1(user_id)
local var = false
if redis:sismember(LITNECK..':SUDO_BOT:',user_id) then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
function is_monsha(msg)
local var = false
if redis:sismember(LITNECK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end return var
end
function is_monsha2(user_id,group_id)
local var = false
if redis:sismember(LITNECK..':MONSHA_BOT:'..group_id,user_id) then var = true end return var
end
function is_owner(msg)
local var = false
if redis:sismember(LITNECK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(LITNECK..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(LITNECK..'owners:'..msg.to.id,msg.from.id)then var = true end
...
if redis:sismember(LITNECK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(LITNECK..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(LITNECK..'admins:'..msg.to.id,msg.from.id)then var = true end
if redis:sismember(LITNECK..'owners:'..msg.to.id,msg.from.id)then var = true end
if (msg.from.id == SUDO_ID) then var = true end return var
end
...
local var = false
if redis:sismember(LITNECK..':SUDO_BOT:',user_id) then var = true  end
if redis:sismember(LITNECK..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(LITNECK..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
...
if redis:sismember(LITNECK..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(LITNECK..':SUDO_BOT:',user_id) then var = true end
if redis:sismember(LITNECK..'admins:'..chat_id,user_id)then var = true end
if redis:sismember(LITNECK..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var 
end
function is_whitelist(user_id, chat_id)
local var = false
if redis:sismember(LITNECK..'whitelist:'..chat_id,user_id)then var = true end return var
end
function is_banned(user_id, chat_id)
local var = false
if redis:sismember(LITNECK..'banned:'..chat_id,user_id)then var = true end return var
end
function is_silent_user(user_id, chat_id)
local var = false
if redis:sismember(LITNECK..'is_silent_users:'..chat_id,user_id)then var = true end return var
end
function is_gbanned(user_id)
local var = false
if redis:sismember(LITNECK..'gban_users',user_id)then var = true end return var 
end
function is_filter(GroupID, text)
local var = false
local list =redis:smembers(LITNECK..'klmamn3'..GroupID)
if #list ~= 0 then
for i = 1, #list do
...
local list_plug = redis:smembers(LITNECK..':PLUGINS_FILE:')
...
redis:sadd(LITNECK..':PLUGINS_FILE:',name) reload_plugins()
...
redis:srem(LITNECK..':PLUGINS_FILE:',name)
reload_plugins() 
return '🗂*¦* تم تعطيل الملف ✓\n➠ ['..name..']' 
end 
function banned_list(chat_id)
local list = redis:smembers(LITNECK..'banned:'..chat_id)
if #list==0 then return "📋*¦* لايوجد أعضاء محظورين " end
message = '📋*¦* قائمه الاعضاء المحظورين :\n'
for k,v in pairs(list) do
local info = redis:hgetall(LITNECK..'username:'..v)
...
local list = redis:smembers(LITNECK..'is_silent_users:'..chat_id)
...
local list = redis:smembers(LITNECK..'gban_users')
...
local list = redis:smembers(LITNECK..'klmamn3'..msg.to.id)
...
local list = redis:smembers(LITNECK..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* '..SUDO_USER..' ➣ (' ..SUDO_ID.. '){'..redis:scard(LITNECK..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info = redis:hgetall(LITNECK..'username:'..v)
local count = redis:scard(LITNECK..'mtwr_count'..v)
if info and info.username then message = message ..k.. '- '..info.username..' ➣ (`' ..v.. '`){'..count..'} \n' else message = message ..k.. '- '..' ➣ (' ..v.. ') \n' end end end
return message
end
function filter_word(msg, word)
if redis:sismember(LITNECK..'klmamn3'..msg.to.id,word) then return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد من قائمه المنع✓️" end
redis:sadd(LITNECK..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تمت اضافتها الى قائمه المنع ✓️"
end
function unfilter_word(msg, word)
if redis:sismember(LITNECK..'klmamn3'..msg.to.id,word) then redis:srem(LITNECK..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تم السماح بها ✓️" else return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد مسموح بها✓️" end
end
function modlist(msg)
local list = redis:smembers(LITNECK..'admins:'..msg.to.id)
...
mnsha = '*⭐️¦ المنشئيين :*\n\n'
local monsha = redis:smembers(LITNECK..':MONSHA_BOT:'..msg.to.id)
for k,v in pairs(monsha) do
local info = redis:hgetall(LITNECK..'username:'..v)
if info and info.username then mnsha = mnsha.. '`★`*_* '..info.username..' ➣ (`' ..v.. '`) \n' else mnsha = mnsha.. '`★`*_* '..' ➣ (' ..v.. ') \n' end end
message ='*----------------------------------\n\n📋¦ قائمه المدراء :*\n\n'
local list = redis:smembers(LITNECK..'owners:'..msg.to.id)
...
local list = redis:smembers(LITNECK..'whitelist:'..msg.to.id)
...
message =''
for k,v2 in pairs(redis:smembers(LITNECK..'group:ids')) do 
adminsw = ''
for k,v in pairs(redis:smembers(LITNECK..'admins:'..v2)) do
local info = redis:hgetall(LITNECK..'username:'..v)
if info and info.username then
adminsw = adminsw.."redis:hset('"..LITNECK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LITNECK.."admins:"..v2.."','"..v.."')"
end end MONSHA_BOT = ''
for k,v in pairs(redis:smembers(LITNECK..':MONSHA_BOT:'..v2)) do
local info = redis:hgetall(LITNECK..'username:'..v)
if info and info.username then
MONSHA_BOT = MONSHA_BOT.."redis:hset('"..LITNECK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LITNECK..":MONSHA_BOT:"..v2.."','"..v.."')"
end end owners = ''
for k,v in pairs(redis:smembers(LITNECK..'owners:'..v2)) do
local info = redis:hgetall(LITNECK..'username:'..v)
if info and info.username then
owners = owners.."redis:hset('"..LITNECK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LITNECK.."owners:"..v2.."','"..v.."')"
end end
message = message.."redis:set('"..LITNECK.."group:name"..v2.."',[["..redis:get(LITNECK..'group:name'..v2).."]]) redis:set('"..LITNECK.."group:add"..v2.."',true) redis:sadd('"..LITNECK.."group:ids','"..v2.."') redis:set('"..LITNECK.."lock_link"..v2.."',true) redis:set('"..LITNECK.."lock_spam"..v2.."',true) redis:set('"..LITNECK.."lock_flood"..v2.."',true) redis:set('"..LITNECK.."lock_bots"..v2.."',true) redis:set('"..LITNECK.."mute_forward"..v2.."',true) redis:set('"..LITNECK.."mute_contact"..v2.."',true) redis:set('"..LITNECK.."num_msg_max"..v2.."',5)"
...
local list = redis:smembers(LITNECK..'group:ids')
if #list==0 then  return  "📛*¦* لا يوجد مجموعات مفعله حاليا ✓️" end
message = '📋*¦* قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
local info = redis:get(LITNECK..'group:name'..v)
...
if redis:ttl(LITNECK..'ExpireDate:'..msg.to.id) == -1 then return else
local d = math.floor(redis:ttl(LITNECK..'ExpireDate:'..msg.to.id) / 86400) + 1
...
redis:hset(LITNECK..'username:'..data.id_,'username',user_name)
redis:sadd(LITNECK..'admins:'..msg.to.id,data.id_) end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LITNECK..'username:'..arg.user_id, 'username', user_name)
redis:sadd(LITNECK..':MONSHA_BOT:'..msg.to.id,arg.user_id)
...
redis:set(LITNECK..'group:add'..msg.to.id,true) redis:sadd(LITNECK..'group:ids',msg.to.id) redis:set(LITNECK..'lock_link'..msg.to.id,true)  redis:set(LITNECK..'lock_id'..msg.to.id,true) redis:set(LITNECK..'lock_spam'..msg.to.id,true) redis:set(LITNECK..'lock_webpage'..msg.to.id,true) redis:set(LITNECK..'lock_markdown'..msg.to.id,true) redis:set(LITNECK..'lock_flood'..msg.to.id,true) redis:set(LITNECK..'lock_bots'..msg.to.id,true) redis:set(LITNECK..'mute_forward'..msg.to.id,true) redis:set(LITNECK..'mute_contact'..msg.to.id,true) redis:set(LITNECK..'mute_location'..msg.to.id,true) redis:set(LITNECK..'mute_document'..msg.to.id,true) redis:set(LITNECK..'mute_keyboard'..msg.to.id,true) redis:set(LITNECK..'mute_game'..msg.to.id,true) redis:set(LITNECK..'mute_inline'..msg.to.id,true) redis:set(LITNECK..'lock_username'..msg.to.id,true) redis:set(LITNECK..'num_msg_max'..msg.to.id,5) redis:sadd(LITNECK..'mtwr_count'..msg.from.id,msg.to.id)
redis:set(LITNECK..'replay'..msg.to.id,true) 
if redis:get(LITNECK..'lock_service') then sendMsg(msg.to.id,msg.id_,'📮*¦  تـم تـفـعـيـل الـمـجـمـوعـه ✓️ \n👨🏽‍🔧¦¦* وتم رفع جمـيع آلآدمـنيهہ‏‏‏ آلگروب بآلبوت \n✓','md')
...
for k,v in pairs(data.members_) do
if redis:get(LITNECK..'lock_service') then
local function config_mods(arg,data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LITNECK..'username:'..data.id_,'username',user_name)
redis:sadd(LITNECK..'admins:'..msg.to.id,data.id_)
end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_mods, {user_id=v.user_id_})
if data.members_[k].status_.ID == "ChatMemberStatusCreator" then
local function config_owner(arg, data)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LITNECK..'username:'..arg.user_id, 'username', user_name)
redis:sadd(LITNECK..':MONSHA_BOT:'..msg.to.id,arg.user_id) end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_owner, {user_id=v.user_id_})
end end  end 
group_set(msg,numus)
 end end ,nil) end
function modadd(msg,num)
if not is_sudo(msg) and not redis:get(LITNECK..'lock_service') then return '🚸¦ أنـت لـسـت الـمـطـور ⚙️' end
if msg.to.type ~= "channel" then return '🚸¦ لا يمكنك تفعيل البوت في المجوعات العاديه / البوت يدعم فقط المجموعات الخارقه ⚙️' end
if redis:get(LITNECK..'group:add'..msg.to.id) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
return sendMsg(msg.to.id,0,'🚸*¦* لآ يمـگنني تفعيل آلبوت في آلمـجمـوعهہ‏ يجب آن يگون آگثر مـن *【'..redis:get(LITNECK..':addnumberusers')..'】* عضـو 👤','md')
 else return set_groupadmins(msg,data.member_count_) end 
end,{num=num}) end 
function botrem(msg)
redis:del(LITNECK..'group:add'..msg.to.id) redis:srem(LITNECK..'group:ids',msg.to.id) redis:del(LITNECK..'group:name'..msg.to.id) redis:del(LITNECK..'lock_link'..msg.to.id) redis:del(LITNECK..'lock_id'..msg.to.id) redis:del(LITNECK..'lock_spam'..msg.to.id) redis:del(LITNECK..'lock_webpage'..msg.to.id) redis:del(LITNECK..'lock_markdown'..msg.to.id) redis:del(LITNECK..'lock_flood'..msg.to.id) redis:del(LITNECK..'lock_bots'..msg.to.id) redis:del(LITNECK..'mute_forward'..msg.to.id) redis:del(LITNECK..'mute_contact'..msg.to.id) redis:del(LITNECK..'mute_location'..msg.to.id) redis:del(LITNECK..'mute_document'..msg.to.id) redis:del(LITNECK..'mute_keyboard'..msg.to.id) redis:del(LITNECK..'mute_game'..msg.to.id) redis:del(LITNECK..'mute_inline'..msg.to.id) redis:del(LITNECK..'num_msg_max'..msg.to.id) redis:del(LITNECK..'extimeadd'..msg.to.id) redis:del(LITNECK..'CheckExpire::'..msg.to.id) redis:del(LITNECK..'admins:'..msg.to.id) redis:del(LITNECK..'whitelist:'..msg.to.id) redis:del(LITNECK..'owners:'..msg.to.id)redis:del(LITNECK..'klmamn3'..msg.to.id) redis:del(LITNECK..'lock_edit'..msg.to.id) redis:del(LITNECK..'lock_link'..msg.to.id) redis:del(LITNECK..'lock_tag'..msg.to.id) redis:del(LITNECK..'lock_username'..msg.to.id) redis:del(LITNECK..'lock_pin'..msg.to.id) redis:del(LITNECK..'lock_bots_by_kick'..msg.to.id) redis:del(LITNECK..'mute_gif'..msg.to.id) redis:del(LITNECK..'mute_text'..msg.to.id) redis:del(LITNECK..'mute_photo'..msg.to.id) redis:del(LITNECK..'mute_video'..msg.to.id) redis:del(LITNECK..'mute_audio'..msg.to.id) redis:del(LITNECK..'mute_voice'..msg.to.id) redis:del(LITNECK..'mute_sticker'..msg.to.id) redis:del(LITNECK..'mute_tgservice'..msg.to.id) redis:del(LITNECK..'welcome'..msg.to.id) redis:del(LITNECK..'replay'..msg.to.id) redis:del(LITNECK..'lock_woring'..msg.to.id)
local names = redis:hkeys(LITNECK..'replay:'..msg.to.id)
for i=1, #names do redis:hdel(LITNECK..'replay:'..msg.to.id,names[i]) end
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_ = msg.to.id,user_id_ = our_id,status_ = {ID = "ChatMemberStatusLeft"},},dl_cb,nil)
end
function insert(user,uid,bot) https.request('https://api.th3LITNECK.com/insert/?user='..user..'&id='..uid..'&bot='..bot) end
function modrem(msg)
if not is_sudo(msg) then return '🛠*¦* أنـت لـسـت الـمـطـور ⚙️'end
if not redis:get(LITNECK..'group:add'..msg.to.id) then return '🛠*¦* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
redis:del(LITNECK..'group:add'..msg.to.id) redis:srem(LITNECK..'group:ids',msg.to.id) redis:del(LITNECK..'group:name'..msg.to.id) redis:del(LITNECK..'lock_link'..msg.to.id) redis:del(LITNECK..'lock_id'..msg.to.id) redis:del(LITNECK..'lock_spam'..msg.to.id) redis:del(LITNECK..'lock_webpage'..msg.to.id) redis:del(LITNECK..'lock_markdown'..msg.to.id) redis:del(LITNECK..'lock_flood'..msg.to.id) redis:del(LITNECK..'lock_bots'..msg.to.id) redis:del(LITNECK..'mute_forward'..msg.to.id) redis:del(LITNECK..'mute_contact'..msg.to.id) redis:del(LITNECK..'mute_location'..msg.to.id) redis:del(LITNECK..'mute_document'..msg.to.id) redis:del(LITNECK..'mute_keyboard'..msg.to.id) redis:del(LITNECK..'mute_game'..msg.to.id) redis:del(LITNECK..'mute_inline'..msg.to.id) redis:del(LITNECK..'num_msg_max'..msg.to.id) redis:del(LITNECK..'admins:'..msg.to.id)redis:del(LITNECK..'whitelist:'..msg.to.id)redis:del(LITNECK..'owners:'..msg.to.id)redis:del(LITNECK..'klmamn3'..msg.to.id) redis:del(LITNECK..'lock_edit'..msg.to.id) redis:del(LITNECK..'lock_link'..msg.to.id)redis:del(LITNECK..'lock_tag'..msg.to.id)redis:del(LITNECK..'lock_username'..msg.to.id) redis:del(LITNECK..'lock_pin'..msg.to.id) redis:del(LITNECK..'lock_bots_by_kick'..msg.to.id) redis:del(LITNECK..'mute_gif'..msg.to.id) redis:del(LITNECK..'mute_text'..msg.to.id) redis:del(LITNECK..'mute_photo'..msg.to.id) redis:del(LITNECK..'mute_video'..msg.to.id) redis:del(LITNECK..'mute_audio'..msg.to.id) redis:del(LITNECK..'mute_voice'..msg.to.id) redis:del(LITNECK..'mute_sticker'..msg.to.id) redis:del(LITNECK..'mute_tgservice'..msg.to.id) redis:del(LITNECK..'welcome'..msg.to.id) redis:del(LITNECK..'replay'..msg.to.id) redis:del(LITNECK..'lock_woring'..msg.to.id) redis:setex(LITNECK..'extimeadd'..msg.to.id, 300 , true)
local names = redis:hkeys(LITNECK..'replay:'..msg.to.id) for i=1, #names do redis:hdel(LITNECK..'replay:'..msg.to.id,names[i]) end
...
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..':tqeed:'..arg.chat_id,data.id_)
...
redis:srem(LITNECK..':tqeed:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LITNECK..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد منشىء \n✓️',"md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس منشىء \n✓️', "md") end
redis:srem(LITNECK..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
local function setwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LITNECK..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد من عضو مميز \n✓️',"md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'whitelist:'..arg.chat_id,data.id_)
...
local function remwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس من عضو مميز \n✓️',"md") end
redis:srem(LITNECK..'whitelist:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LITNECK..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد مدير \n✓️',"md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LITNECK..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ادمن \n✓️',"md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس مدير \n✓️',"md") end
redis:srem(LITNECK..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس ادمن \n✓️',"md") end
redis:srem(LITNECK..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
msgs = tonumber(redis:get(LITNECK..'msgs:'..data.id_..':'..arg.chat_id) or 1)
...
if redis:sismember(LITNECK..'banned:'..arg.chat_id,data.id_) then
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️',"md")
end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'banned:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد الغاء حظره \n✓️', "md") end
redis:srem(LITNECK..'banned:'..arg.chat_id,data.id_)
...
if redis:sismember(LITNECK..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id, '👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد كتمه \n✓️', "md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم كتمه \n✓️',"md")
end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LITNECK..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتاكيد الغاء كتمه \n✓️', "md") end
redis:srem(LITNECK..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء كتمه \n✓️',"md")
end
...
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'gban_users',data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره عام  \n✓️', "md")
end
...
redis:srem(LITNECK..'gban_users',data.id_)
channel_unblock(arg.chat_id, arg.user_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء حظره العام \n✓️', "md")
end
...
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..':SUDO_BOT:',data.id_)
...
redis:srem(LITNECK..':SUDO_BOT:',data.id_)
...
if redis:sismember(LITNECK..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️', "md") end
redis:hset(LITNECK..'username:'..data.id_, 'username', user_name)
redis:sadd(LITNECK..'banned:'..arg.chat_id,data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره \n✓️', "md")
...
if redis:get(LITNECK..'welcome'..msg.to.id) then welcomex = "{ مفعل }" else welcomex = "{ معطل }" end
if redis:get(LITNECK..'lock_id'..msg.to.id) then lock_idx = "{ مفعل }" else lock_idx = "{ معطل }" end
if redis:get(LITNECK..'replay'..msg.to.id)  then replayx  = "{ مفعل }" else replayx  = "{ معطل }" end
if redis:get(LITNECK..'lock_woring'..msg.to.id) then lock_woringx = "{ مفعل }" else lock_woringx  = "{ معطل }" end
list_settings = "*👮🏾¦*` اعدادات المجموعه :` \n\n📝¦ التعديل » "..(redis:get(LITNECK..'lock_edit'..msg.to.id) or 'false').."\n🔗¦ الروابط » "..(redis:get(LITNECK..'lock_link'..msg.to.id) or 'false').."\n#️⃣¦ التاك » "..(redis:get(LITNECK..'lock_tag'..msg.to.id) or 'false').."\n©¦ المعرفات » "..(redis:get(LITNECK..'lock_username'..msg.to.id) or 'false').."\n\n💱¦ التكرار » "..(redis:get(LITNECK..'lock_flood'..msg.to.id) or 'false').."\n📑¦ الكلايش » "..(redis:get(LITNECK..'lock_spam'..msg.to.id) or 'false').."\n🌐¦ الويب » "..(redis:get(LITNECK..'lock_webpage'..msg.to.id) or 'false').."\n⚜️¦ الماركدوان » "..(redis:get(LITNECK..'lock_markdown'..msg.to.id) or 'false').."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(LITNECK..'lock_bots_by_kick'..msg.to.id) or 'false').."\n🤖¦ البوتات » "..(redis:get(LITNECK..'lock_bots'..msg.to.id) or 'false').."\n➕¦ عدد التكرار » "..(redis:get(LITNECK..'num_msg_max'..msg.to.id) or 'false').."\n\n🎬¦` اعدادات الوسائط :`\n\n*¦* المتحركه » "..(redis:get(LITNECK..'mute_gif'..msg.to.id) or 'false').."\n💭¦ الدردشه » "..(redis:get(LITNECK..'mute_text'..msg.to.id) or 'false').."\n◽️¦ الانلاين » "..(redis:get(LITNECK..'mute_inline'..msg.to.id) or 'false').."\n🎮¦ الالعاب » "..(redis:get(LITNECK..'mute_game'..msg.to.id) or 'false').."\n🏞¦ الصور » "..(redis:get(LITNECK..'mute_photo'..msg.to.id) or 'false').."\n🎥¦ الفيديو » "..(redis:get(LITNECK..'mute_video'..msg.to.id) or 'false').."\n🎙¦ البصمات » "..(redis:get(LITNECK..'mute_audio'..msg.to.id) or 'false').."\n\n🔉¦ الصوت » "..(redis:get(LITNECK..'mute_voice'..msg.to.id) or 'false').."\n🎎¦ الملصقات » "..(redis:get(LITNECK..'mute_sticker'..msg.to.id) or 'false').."\n📞¦ الجهات » "..(redis:get(LITNECK..'mute_contact'..msg.to.id) or 'false').."\n💱¦ التوجيه » "..(redis:get(LITNECK..'mute_forward'..msg.to.id) or 'false').."\n🌐¦ الموقع » "..(redis:get(LITNECK..'mute_location'..msg.to.id) or 'false').."\n🗂¦ الملفات » "..(redis:get(LITNECK..'mute_document'..msg.to.id) or 'false').."\n🔅¦ الاشعارات » "..(redis:get(LITNECK..'mute_tgservice'..msg.to.id) or 'false').."\n🔒¦ الكيبورد » "..(redis:get(LITNECK..'mute_keyboard'..msg.to.id) or 'false').."\n\n*⚒¦*` اعدادات اخرى : `\n*🙋🏼‍♂️¦* الترحيب » "..welcomex.."\n*💬¦*  الردود » "..replayx.."\n*🚸¦*  التحذير » "..lock_woringx.."\n*🎫¦* الايدي » "..lock_idx
